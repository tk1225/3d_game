...
void draw(t_vars *vars, int x)
{
  int ceil = 0;
    ...
    while (vars->drawStart < vars->drawEnd)
    {
        ...
        // switch(vars->tmpimg[vars->texNum][TEXHEIGHT * vars->texY + vars->texX])
        // {
        ...
        mlx_pixel_put(vars->mlx, vars->win, x, vars->drawStart, vars->tmpimg[vars->texNum][TEXHEIGHT * vars->texY + vars->texX]);
        vars->drawStart ++;
    }
